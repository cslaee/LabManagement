...
/*
  ...
  <x:Stylesheet HRef="schedule_files/stylesheet.css"/>
  <x:WindowHeight>12375</x:WindowHeight>
  <x:WindowWidth>16320</x:WindowWidth>
  <x:WindowTopX>5250</x:WindowTopX>
  <x:WindowTopY>75</x:WindowTopY>
  <x:TabRatio>734</x:TabRatio>
...
*/
            ...
            using (FileStream fs = new FileStream(Constants.webpageDir + sheetName + ".htm", FileMode.Create))
            {
                using (StreamWriter w = new StreamWriter(fs, Encoding.UTF8))
                {
                    ...
                    /*
                       <x:ExcelWorksheet>
                        <x:Name>F19</x:Name>
                        <x:WorksheetSource HRef="schedule_files/sheet001.htm"/>
                       </x:ExcelWorksheet>
                    */
                    i = 1;
                    foreach (string tabName in semesterNames)
        ...
        static public void StoreIndexOld(string[] semesterNames)
        {
            #region html Content Strings
